...
class Client(local):
    ...
    def get_stats(self, stat_args = None):
        ...
        return(data)
    def get_dump_key(self):
        """ get all keys this only for one socket ,if more need to motify """
        data_dump = self.get_slabs();
        data = data_dump[0];
        data_name = data[0];
        data_dict = data[1];
        out_data = [];
        ...
        for k in data_dict:
            ...
            for s in self.servers:
                ...
                if s.family == socket.AF_INET:
                    ...
                    name = 'unix:%s (%s)' % ( s.address, s.weight )
                serverData = {}
                out_data.append(( name, serverData ))
                #print("key:",k);
                s.send_cmd('stats cachedump '+ k +' 0');
                readline = s.readline
                while 1:
                    line = readline();
                    if (not line or line.strip() == 'END'): break;
                    item = line.split(' ');
                    #ITEM bb [5 b; 13844107083 s] 1394391894 1384410704
                    # 0    1  2 3  4  5
                    item_key = item[1];
                    #print("item_key:",item_key);
                    item_size_len = len(item[2]);
                    item_size = item[2][1:item_size_len];
                    item_end = item[4];
                    serverData[item_key] = {'size':item_size,'end_time':item_end};
        return (out_data,data_dump)
    def get_slabs(self):
        """
